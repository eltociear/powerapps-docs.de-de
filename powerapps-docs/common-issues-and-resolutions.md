---
title: "Häufige Probleme und Lösungen | Microsoft-Dokumentation"
...
**Hinweis**: Weitere Informationen finden Sie in der [Liste der bekannten Probleme](https://docs.microsoft.com/en-us/common-data-service/entity-reference/known-issues) für den Microsoft Common Data Service.

## <a name="recently-addedchanged"></a>Kürzlich hinzugefügt/geändert
1. **Kamerasteuerelemente in PowerApps Studio für Windows**
   
    Aufgrund eines Windows-Updates stürzt PowerApps Studio für Windows möglicherweise ab, wenn Sie ein Kamerasteuerelement hinzufügen oder eine App öffnen, die ein Kamerasteuerelement verwendet. Um dieses Problem zu vermeiden, verwenden Sie [PowerApps Studio for Web](create-app-browser.md), wenn Sie ein Kamerasteuerelement hinzufügen oder nutzen.
2. **Version 2.0.700 auf Android-Geräten**
...
3. **„Leerer“ Katalog beim Öffnen einer App**
...
4. **Aktualisieren von PowerApps unter Windows 8.1**
...
2. **Verwenden einer lokalisierten Version**.
...
3. **Kamerasteuerelement eines Windows Phones**
...
4. **Öffnen einer App aus einer Vorlage**
...
6. **Erstellen und Bearbeiten von Apps in einem Browser**
...
9. **Wenn PowerApps aus Daten eine App generiert, wird das Feld zum Sortieren und Durchsuchen nicht automatisch konfiguriert**.
...
10. **Bei Apps, die aus Daten erstellt werden, kann nur auf die ersten 500 Datensätze einer Datenquelle zugegriffen werden**.
...
11. **Excel-Daten müssen als Tabelle formatiert sein**.
...
12. **Benutzerdefinierte SharePoint-Listen werden unterstützt, Bibliotheken jedoch nicht, bestimmte Typen von Listenspalten oder Spalten, die mehrere Werte oder Auswahlen unterstützen**.
    
     Weitere Informationen finden Sie unter [SharePoint Online](connections/connection-sharepoint-online.md#known-issues).
13. **Die gemeinsame Erstellung wird nicht unterstützt. Bitte immer nur ein Autor zur gleichen Zeit**.
...
14. **Manchmal kann es einen Moment dauern, bis eine neu freigegebene App verwendet werden kann**.
...
15. **Im [Formularsteuerelement](controls/control-form-detail.md), können Sie Daten nicht mithilfe einer benutzerdefinierten Karte ändern**.
...
16. **Eine App, die auf Android 5.0, Nexus 6 mit den WebView-Versionen 48 oder 49 ausgeführt wird, kann abstürzen**.
...
17. **Die Kameraverwendung kann vorübergehend deaktiviert sein, wenn der Speicherplatz gering ist**.
...
18. **Der Office 365-Video-Connector wird nicht unterstützt**.
19. **Der Kartenkatalog wird nicht mehr unterstützt**.
