...
|Domäne|Gibt die Domäne an, die Anmeldeinformationen von Benutzern überprüft.|  
|UserName, Benutzername, UserId oder Benutzer-ID|Gibt die Benutzerkennung an, die den Anmeldeinformationen zugeordnet ist.|  
|Kennwort|Gibt das Kennwort für den Benutzernamen, das den Anmeldeinformationen zugeordnet ist.|  
...
|CertStoreName|Gibt den Namen des Orts auf dem Computer an, auf dem sich das Zertifikat mit dem übergebenen Fingerabdruck befindet. Dieser Parameter gilt nur, wenn `Certificate` als Authentifizierungstyp angegeben ist.|
|CertThumbPrint| Gibt den Fingerabdruck des Zertifikats an, der verwendet werden soll. Dieser Parameter gilt nur, wenn `Certificate` als Authentifizierungstyp angegeben ist.|
|SkipDiscovery|Wenn der Parameter auf „true” festgelegt ist, sollte die Service-URI direkt verwendet werden.| 
