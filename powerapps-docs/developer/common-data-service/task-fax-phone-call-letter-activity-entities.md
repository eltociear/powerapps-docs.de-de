---
title: 'Aufgaben-, Fax-, Telefonanruf- und Briefaktivitäten-Entitäten (Common Data Service for Apps) | Microsoft Docs'
description: 'Erfahren Sie mehr über Entitäten, die es Ihnen ermöglichen, Hauptaktivitäten zu verwalten, die in einer Beziehungsverwaltungslösung ausgeführt werden.'
ms.custom: ''
ms.date: 10/31/2018
ms.reviewer: ''
ms.service: powerapps
ms.topic: article
author: mayadumesh
ms.author: jdaly
manager: ryjones
search.audienceType:
  - developer
search.app:
  - PowerApps
  - D365CE
---
# <a name="task-fax-phone-call-and-letter-activity-entities"></a>Aktivitätsentitäten wie Aufgabe, Fax, Telefonanruf und Brief

Dieser Abschnitt enthält Informationen über die Entitäten, die es Ihnen ermöglichen, Hauptaktivitäten zu verwalten, die in einer Beziehungsverwaltungslösung ausgeführt werden.  
## <a name="in-this-section"></a>In diesem Abschnitt  
 [Fax-Entität](reference/entities/fax.md)  
  
 [Schreibenentität](reference/entities/letter.md)  
  
 [Telefonanrufsentität](reference/entities/phonecall.md)  
  
 [Aufgabenentität](reference/entities/task.md)  
  
### <a name="see-also"></a>Siehe auch  
 [Aktivitätsentitäten](activity-entities.md) 