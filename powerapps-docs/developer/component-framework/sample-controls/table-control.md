...
Diese Komponente demonstriert außerdem die zwei Dienstprogrammfunktionen `setFullScreen` und `lookupObjects`. Diese Funktionen werden ausgelöst, indem Sie auf die Schaltfläche klicken, die im Rahmen der benutzerdefinierten Komponente gerendert wird. Die `setFullScreen`-Schaltfläche schaltet die Komponente in oder aus dem Vollbildmodus. Die `lookupObjects`-Schaltfläche öffnet einen Suchdialog und bringt dann den ausgewählten Datensatz als Text in div ein.
...
Die Rückrufmethode bringt diese Informationen als HTML in ein auf der Komponente gerendertes div ein, um dem Benutzer die ausgewählten Ergebnisse anzuzeigen. Wenn das Promise abgelehnt wird, wird die Fehler-Rückrufmethode ausgelöst, wo Ihre Komponente das Fehlerszenario entsprechend behandeln kann.
