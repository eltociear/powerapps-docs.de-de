...
In diesem Beispiel wird gezeigt, wie Sie React verwenden, um Steuerelemente mithilfe des PowerApps component framework zu erstellen.  Die Facepile-Beispielkomponente wird anhand von React und der Office UI Fabric React-Komponenten implementiert. Dieser Code deckt möglicherweise nicht die bewährten Methoden für die erwähnten Drittanbieterbibliotheken ab.

> [!div class="mx-imgBorder"]
> ![Facepile in React](../media/react-facepile.png "Facepile in React")

## <a name="a-note-on-performance"></a>Ein Hinweis zur Leistung

Obwohl die PowerApps-Hostanwendungen zusätzlich zu React arbeiten, wird die Version von React, die Sie als Paket erstellen, weder mit der Hostversion kommunizieren noch ist sie von dieser Version abhängig. Eine neue Kopie von React (oder eine Drittanbieter-Bibliothek, die Sie mit Ihrem Steuerelement als Paket erstellen) wird jede Instanz dieses Steuerelements auf die Hostseite geladen. Achten Sie daher darauf, wie groß Sie Ihre Seite(n) machen, wenn Sie Steuerelemente hinzufügen. In einer künftigen Version werden wir eine Lösung zu diesem Problem haben.
...
Dieses Beispiel enthält Beispiele darüber, wie Sie Abhängigkeiten für Drittanbieter-Bibliotheken und Office UI Fabric hinzufügen, und zeigt, wie Sie die Office UI Fabric-Komponenten für React für die UI und eine bidirektionale Datenbindung zwischen dem PowerApps component framework und dem React-Statusmodell verwenden.

Das Komponentenbeispiel besteht aus drei Office UI Fabric-Komponenten: ein Facepile, ein Schieberegler, ein Kontrollkästchen und eine Dropdownliste. Wenn Sie den Schieberegler bewegen, ändert sich die Anzahl der Gesichter in der Facepile. Das Kontrollkästchen legt fest, ob die Gesichter ein- und ausgeblendet oder einfach erscheinen und wieder verschwinden. Die Optionen in der Dropdownliste legen die Größe der Gesichter fest. Wenn kein Wert festgelegt ist, ist die Anzahl der Gesichter standardmäßig 3.
...
- Ein Ereignishandler wird an die Eigenschaften der React-Komponente übergeben. Dadurch kann die React-Komponente das Host-Steuerelement des PowerApps component framework benachrichtigen, dass ein Wert sich geändert hat. Der Ereignishandler legt dann fest, ob ein Aufruf der **notifyOutputEvents**-Methode erforderlich ist.
- Durch das Bewegen des Schiebereglers aktualisiert React den Wert mit Bindung und ruft den weitergegebenen Ereignishandler auf. Wenn eine **notifyOutputEvents**-Methode innerhalb eines Handlers aufgerufen wird, wird die [getOutputs](../reference/control/getoutputs.md)-Methode des Steuerelements asynchron aufgerufen und zum PowerApps component framework weitergegeben. 
