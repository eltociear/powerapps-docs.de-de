...
Die `retrieveMultiple`-FetchXML-Methode rendert eine Schalfläche in der benutzerdefinierten Komponente. Bei `onClick` auf diese Schaltfläche wird FetchXML generiert und an die `retrieveMultiple`-Funktion übergeben, um den Durchschnittswert des Umsatzfelds für alle Firmendatensätze zu berechnen. Die Rückrufmethode des Web-API-Aufrufs bringt das Ergebnis des Web-API-Aufrufs in das Ergebnis-div (Erfolg oder Fehler) des benutzerdefinierten Steuerelements ein.  

Die `retrieveMultiple`-OData-Methode rendert eine Schalfläche in der benutzerdefinierten Komponente. Bei `onClick` auf dieser Schaltfläche, wird eine OData-Zeichenfolge generiert und an die `retrieveMultiple`-Funktion übergeben, um alle Firmendatensätze mit einem Namensfeld, z. B. "Benutzerdefinierte Komponenten-Web-API (Beispiel)" abzurufen. Dies gilt für alle Firmendatensätze, die vom Beispiel für benutzerdefinierte Komponenten erstellt wurden.  

Bei erfolgreichem Abruf der Datensätze verfügt die benutzerdefinierte Komponente über die Logik, um Zählen, bei wie vielen der Datensätze das Umsatzfeld auf 100, 200 oder 300 festgelegt wurde und diese Zahl in einem OData-Statuscontainer div der benutzerdefinierten Komponente anzuzeigen.  Die Rückrufmethode des Web-API-Aufrufs bringt das Ergebnis des Web-API-Aufrufs in das Ergebnis-div (Erfolg oder Fehler) des benutzerdefinierten Steuerelements ein.  
