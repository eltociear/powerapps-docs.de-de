...
Sie können auch die **Item**-Eigenschaft eines Formulars mithilfe eines **Dropdown**-Steuerelements wie unter [Show, edit, or add a record (Anzeigen, Bearbeiten oder Hinzufügen eines Datensatzes)](../add-form.md) beschrieben festlegen. Alternativ können Sie Funktionen wie **Lookup** oder **First** verwenden. Beispielsweise können Sie die **Item** -Eigenschaft auf eine dieser Formeln festlegen, um den Fabrikam-Eintrag in der Entität **Accounts** in Common Data Service anzuzeigen:
...
| **FormMode.New** |Der Benutzer kann einen Datensatz mithilfe des Formulars erstellen. Die Werte in den Steuerelementen des Formulars werden vorab mit den Standardwerten für einen Datensatz der Datenquelle aufgefüllt. Wenn die **[SubmitForm](../functions/function-form.md)** -Funktion erfolgreich ausgeführt wird, wird ein Datensatz erstellt. |
| **FormMode.View** |Der Benutzer kann einen Datensatz mithilfe des Formulars anzeigen. Die Werte in den Steuerelementen des Formulars werden vorab mit den Standardwerten für einen Datensatz der Datenquelle aufgefüllt. |
