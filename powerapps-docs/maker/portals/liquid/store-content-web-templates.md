...
Web Template ist eine PowerApps-Entität (adx\_webtemplate), die in "PowerApps Portale enthalten ist und zum Speichern von Template-Quelltexten verwendet wird. Ein Webvorlage wird in der Regel Liquid für die dynamische Inhaltsdarstellung enthalten und ist die zentrale Instanz, mit der Liquid Templates mit dem Rest des PowerApps Portaleystems integriert werden.
...
Webvorlage können in Verbindung mit Seitenvorlagen verwendet werden, um neue Vorlagen für das Content Management System PowerApps Portale zu erstellen. Dies kann komplett in PowerApps durchgeführt werden, ohne, dass .NET-Code geschrieben oder die Portalanwendung neu bereitstellt werden muss.
...
Der häufigste Einsatzfall für Webvorlagen ist das Rendern von HTML. Das Rendern der gesamten Antwort (über das Deaktivieren von **Kopf- und Fußzeile der Website verwenden**) bietet Ihnen jedoch auch die Option alle gewünschten textbasierten Formate zu rendern. Hier wird das Attribut **MIME-Typ** der Webvorlage relevant. Wenn eine Seitenvorlage, die die Websitekopfzeile und -fußzeile nicht verwendet, gerendert wird, wird die Inhaltstyp-Kopfzeile der HTTP-Antwort auf den MIME-Typ der zugeordneten Webvorlage festgelegt. (text/html wird verwendet, wenn kein MIME-Typ bereitgestellt ist.) Dies bietet Ihnen eine breite Auswahl von Optionen zum Rendern von Nicht-HTML-Inhalt mithilfe von Liquid. Ein häufiger Anwendungsfall wäre das Rendern eines [RSS](http://en.wikipedia.org/wiki/RSS)-Feeds, indem ein MIME-Typ auf application/rss+xml festgelegt wird.  
...
Es gibt eine Reihe von vorgefertigten Liquid-Vorlagen, die in PowerApps Portalen verfügbar sind. Um sie zu verwenden, müssen Sie sie über den Namen mithilfe der unten folgenden Liste als Referenz einbinden.
