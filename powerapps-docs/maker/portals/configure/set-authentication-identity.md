...
Zurückkehrende Portalbesucher können sich über lokale Benutzeranmeldeinformationen oder externe Identitätsanbieterkonten authentifizieren. Ein neuer Besucher kann ein neues Benutzerkonto registrieren, indem er einen Benutzernamen und ein Kennwort angibt oder sich über einen externen Anbieter anmeldet. Besucher, die einen Einladungscode vom Portaladministrator erhalten, können den Code bei der Anmeldung für ein neues Benutzerkonto einlösen.
...
|       Authentication/Registration/RememberBrowserEnabled        |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                Aktivieren oder deaktivieren einer "Angemeldet bleiben"-Kontrollkästchen zur Überprüfung der zweiten Stufe (E-Mail-Code), damit die Überprüfung der zweiten Stufe für den aktuellen Browser beibehalten wird. Der Benutzer muss die Zweit-Faktor-Validierung bei der nächsten Anmeldung nicht bestehen, solange derselbe Browser verwendet wird. Standard: true                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
Folgendes beschreibt die Einstellungen, mit denen definiert wird, wann und wie ein Konto für die Authentifizierung gesperrt wird. Wenn eine bestimmte Anzahl von fehlgeschlagenen Passwortversuchen in kurzer Zeit erkannt wird, wird das Benutzerkonto für eine Weile gesperrt. Nach diesem Zeitraum kann der Benutzer erneut eine Anmeldung ausführen.
...
| Authentication/ApplicationCookie/CookieHttpOnly                          | Legt fest, ob der Browser den Zugriff auf das Cookie durch client-seitiges JavaScript zulassen soll. Standard: true. Weitere Informationen: [CookieAuthenticationOptions::CookieHttpOnly](https://msdn.microsoft.com/library/dn385540.aspx).                     |
...
| Authentication/ApplicationCookie/ExpireTimeSpan                          | Steuert, wie lange das Anwendungs-Cookie vom Zeitpunkt seiner Erstellung an gültig bleibt. Standard: 14 Tage. Weitere Informationen: [CookieAuthenticationOptions::ExpireTimeSpan](https://msdn.microsoft.com/library/microsoft.owin.security.cookies.cookieauthenticationoptions.expiretimespan(v=vs.113).aspx).  |
| Authentication/ApplicationCookie/SlidingExpiration                       | SlidingExpiration wird auf true gesetzt, um die Middleware anzuweisen, jedes Mal, wenn sie eine Anforderung verarbeitet, die mehr als die Hälfte des Ablauffensters erreicht hat, ein neues Cookie mit einer neuen Ablaufzeit zu erstellen. Standard: true. Weitere Informationen: [CookieAuthenticationOptions::SlidingExpiration](https://msdn.microsoft.com/library/dn385548.aspx). |
...
| Authentication/ApplicationCookie/LogoutPath                              | Wenn der Abmeldepfad von der Middleware bereitgestellt wird, wird eine Anforderung an diesen Pfad basierend auf dem ReturnUrlParameter umgeleitet. Weitere Informationen: [CookieAuthenticationOptions::LogoutPath](https://msdn.microsoft.com/library/dn385545.aspx).               |
| Authentication/ApplicationCookie/ReturnUrlParameter                      | Der Parameter ReturnUrlParameter bestimmt den Namen des Abfragezeichenfolgen-Parameters, der von der Middleware angehängt wird, wenn ein 401 Unautorisierter Statuscode in einen 302 Redirect auf den Login-Pfad geändert wird. Weitere Informationen: [CookieAuthenticationOptions::ReturnUrlParameter](https://msdn.microsoft.com/library/dn385546.aspx).                           |
...
| Authentication/TwoFactorCookie/ExpireTimeSpan                            | Legt fest, wie lange das Zwei-Faktor-Cookie ab dem Zeitpunkt seiner Erstellung gültig bleibt. Standard: 5 Minuten. Weitere Informationen: [CookieAuthenticationOptions::ExpireTimeSpan](https://msdn.microsoft.com/library/dn385543.aspx).     |
|||
